...
class WPB_YourChannel{
	...
	function __construct(){
		...
		$this->yrc_permission = apply_filters('yourchannel_settings_permission', 'manage_options');
	...
	function canAdminYRC(){
		return current_user_can($this->yrc_permission);
	}
	
	function dieIfNotAllowed(){
		check_ajax_referer('yrc_nonce', 'yrc_nonce');
		if(!$this->canAdminYRC()){
			wp_send_json(['error' => 'permission_error']);
		}
	}
	...
	public static function outputError( $atts, $msg ){
		return '<span id="yrc-wrong-shortcode" style="color:red" data-atts="'.htmlentities(json_encode($atts)).'">'.$msg.'</span>';
	}
	...
	**/
	...
	public function saveLang(){
		$this->dieIfNotAllowed();
		$lang = $_POST['yrc_lang'];
		echo update_option('yrc_lang_terms', $this->validate($lang));
		die();
	}
