...
class Poll_Maker_Ays_Admin {
	...
	public function enqueue_scripts( $hook_suffix ) {
		...
		wp_localize_script('ays-poll-admin-js', 'pollLangObj', array(
			...
			// 'youCanUuseThisShortcode'       => __( 'You can use this shortcode to show your poll.', $this->plugin_name),
			'youCanUuseThisShortcode'       => __( 'Your Poll is Created!<br>Copy the generated shortcode and paste it into any post or page to display Poll', $this->plugin_name),
            'greateJob'                     => __( 'Great job', $this->plugin_name),
            'formMoreDetailed'              => __( 'For more detailed configuration visit', $this->plugin_name),
            'editQuizPage'                  => __( 'edit poll page', $this->plugin_name),
            'done'                        => __( 'Done', $this->plugin_name),
            'thumbsUpGreat'                 => __( 'Thumbs up, Done', $this->plugin_name),
			...
			<div class="poll-maker-challenge">
				<div class="poll-maker-challenge-list-block">
					...
					<ul class="poll-maker-challenge-list">
						<li class="poll-maker-challenge-step-item"><?php echo __('Add a New Poll', $this->plugin_name); ?></li>
						<li class="poll-maker-challenge-step-item"><?php echo __('Name Your Poll', $this->plugin_name); ?></li>
						<li class="poll-maker-challenge-step-item"><?php echo __('Add Options', $this->plugin_name); ?></li>
						<li class="poll-maker-challenge-step-item"><?php echo __('Save the Poll', $this->plugin_name); ?></li>
						<li class="poll-maker-challenge-step-item"><?php echo __('Copy the Shortcode', $this->plugin_name); ?></li>
						<li class="poll-maker-challenge-step-item"><?php echo __('Embed in a Page', $this->plugin_name); ?></li>
					</ul>
				</div>
	...
	public function add_action_links( $links ) {
		...
		$settings_link = array(
			...
			'<a href="https://ays-pro.com/wordpress/poll-maker?utm_source=dashboard&utm_medium=poll-free&utm_campaign=plugins-buy-now-button" class="ays-poll-upgrade-plugin-btn" style="font-weight:bold;color:#01A32A;" target="_blank">' . __('Upgrade 20% Sale', $this->plugin_name) . '</a>',
		);
			...
			$content[] = '<div id="ays-poll-dicount-month" class="ays_poll_dicount_month">';
				...
				$content[] = '<div class="ays-poll-dicount-wrap-box ays-poll-dicount-wrap-text-box">';
					...
					$content[] = '<span class="ays-poll-new-poll-pro-desc">';
						$content[] = '<img class="ays-poll-new-poll-pro-guaranteeicon" src="' . POLL_MAKER_AYS_ADMIN_URL . '/images/poll-maker-guaranteeicon.svg" style="width: 30px;">';
	...
	public function ays_poll_maker_quick_start() {
		...
		$title = stripslashes( $_REQUEST['ays-poll-title'] );
		$question = sanitize_text_field( $_REQUEST['ays_poll_question'] );
		$answers = $_REQUEST['ays-poll-answers'];
		...
		if	($allow_multivote === 'on') {
			$multivote_min_count = $_REQUEST['quick-poll-multivote-min-count'];
			$multivote_max_count = $_REQUEST['quick-poll-multivote-max-count'];
		} else {
		...
		$options = json_encode(
			...
			"answers_grid_column" => 2,"answers_border_radius" => 0,
