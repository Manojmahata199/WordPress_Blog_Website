...
class Polls_List_Table extends WP_List_Table {
	...
	public function extra_tablenav($which) {
        ...
        <div id="poll-filter-div-<?php echo esc_attr( $which ); ?>" class="alignleft actions bulkactions">
			...
			<select name="filterbytype-<?php echo esc_attr( $which ); ?>" id="bulk-action-selector-<?php echo esc_attr( $which ); ?>">
				<option value=""><?php echo __('Select Type',$this->plugin_name)?></option>
				<?php
		...
		<a style="" href="?page=<?php echo esc_attr( $_REQUEST['page'] ); ?>" class="button actions_style"><?php echo __( "Clear filters", $this->plugin_name ); ?></a>
        <?php
    }
	...
	public function add_or_edit_polls( $data, $id = null, $ays_change_type = "" ) {
		...
		if (isset($data["poll_action"]) && wp_verify_nonce($data["poll_action"], 'poll_action')) {
            ...
            $show_chart_type_google_height = (isset($_POST['ays_poll_show_result_chart_google_height']) && sanitize_text_field( $_POST['ays_poll_show_result_chart_google_height'] ) != "" && sanitize_text_field( $_POST['ays_poll_show_result_chart_google_height'] ) != 0) ? absint(intval( sanitize_text_field( $_POST['ays_poll_show_result_chart_google_height'] ) )) : 400;
	...
	function column_title( $item ) {
		...
		$actions = [
			...
			'results'   => sprintf('<a href="?page=%s&poll=%d&title=%s">'. __('View Results', $this->plugin_name) .'</a>', esc_attr( $_REQUEST['page'] ) . '-results-each', absint( $item['id'] ) , $poll_title ),
	...
	function get_columns() {
		$columns = array(
			...
			'results'      		=> __('Results', $this->plugin_name),
