		...
		if ( $activeDateCheck && $activeDeactiveDateCheck && !$is_start_soon) {
		    if (isset($options['ays_poll_show_timer']) && $options['ays_poll_show_timer'] == 1) {
				...
				if ($show_timer_type == 'countdown') {
					...
					$show_timer .= '<p class="show_timer_countdown">'.__('This Poll is active until ',$this->plugin_name).gmdate('jS \of F Y H:i:s', intval($endDate)).'</p>';
				}
				$show_timer .= "</div>";
		    }
		...
		if($user_id != 0){
			...
			if ( ! is_null( $current_user_data ) && $current_user_data ) {
				...
				if( !empty( $user_wordpress_website_url ) ){
					$user_wordpress_website = "<a href='". esc_url( $user_wordpress_website_url ) ."' target='_blank' class='ays-poll-user-website-link-a-tag'>". __( "Website", $this->plugin_name ) ."</a>";
				}
			}
		}
