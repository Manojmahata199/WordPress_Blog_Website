<?php
if ( ! class_exists( 'Ascendoor_Magazine_Posts_List_Widget' ) ) {
	/**
	 * Adds Ascendoor_Magazine_Posts_List_Widget Widget.
	 */
	class Ascendoor_Magazine_Posts_List_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$ascendoor_magazine_list_widget_ops = array(
				'classname'   => 'ascendoor-widget magazine-list-section style-1',
				'description' => __( 'Retrive Posts List Widgets', 'ascendoor-magazine' ),
			);
			parent::__construct(
				'ascendoor_magazine_magazine_list_widget',
				__( 'Ascendoor Posts List Widget', 'ascendoor-magazine' ),
				$ascendoor_magazine_list_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$list_post_count   = isset( $instance['number'] ) ? absint( $instance['number'] ) : 4;
			...
			$list_orderby      = isset( $instance['orderby'] ) && in_array( $instance['orderby'], array( 'title', 'date' ) ) ? $instance['orderby'] : 'date';
			$list_order        = isset( $instance['order'] ) && in_array( $instance['order'], array( 'asc', 'desc' ) ) ? $instance['order'] : 'desc';
			...
			<div class="magazine-section-body">
				<div class="magazine-list-section-wrapper">
					...
					$list_widgets_args = array(
						'post_type'      => 'post',
						'posts_per_page' => absint( $list_post_count ),
		...
		public function form( $instance ) {
			...
			<p>
				...
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" value="<?php echo absint( $list_post_count ); ?>" size="3" />
			</p>
			<p>
			...
			<p>
				...
				<ul>
					<li>
						<label>
							<input id="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'orderby' ) ); ?>" type="radio" value="date" <?php checked( 'date', $list_orderby ); ?> /> 
							<?php esc_html_e( 'Published Date', 'ascendoor-magazine' ); ?>
						</label>
					</li>
					<li>
						<label>
							<input id="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'orderby' ) ); ?>" type="radio" value="title" <?php checked( 'title', $list_orderby ); ?> /> 
			...
			<p>
				...
				<ul>
					<li>
						<label>
							<input id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>" type="radio" value="asc" <?php checked( 'asc', $list_order ); ?> /> 
							<?php esc_html_e( 'Ascending Order', 'ascendoor-magazine' ); ?>
						</label>
					</li>
					<li>
						<label>
							<input id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>" type="radio" value="desc" <?php checked( 'desc', $list_order ); ?> /> 
